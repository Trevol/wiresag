...
class CanvasOverlay(
    ...
    private val onLongPress: OverlayMotionEvent?
) : Overlay() {
    ...
    override fun onLongPress(e: MotionEvent, mapView: MapView) =
        onLongPress?.invoke(OverlayMotionEventScope(e, mapView.projection))
            ?: super.onLongPress(e, mapView)

    override fun draw(canvas: Canvas, pj: Projection) {
        onDraw(DrawScopeImpl(canvas, pj))
    }

    fun evaluateDrawDependencies() {
        onDraw(NoOpDrawScope())
    }

    abstract class DrawScope(val canvas: Canvas) {
    ...
    private class DrawScopeImpl(canvas: Canvas, val projection: Projection) : DrawScope(canvas) {
        override fun IGeoPoint.toPixel(): Point = projection.toPixels(this, Point())
    }

    private class NoOpDrawScope() : DrawScope(NoOpCanvas()) {
...
private class NoOpCanvas : Canvas() {
    ...
    override fun clipOutRect(left: Float, top: Float, right: Float, bottom: Float)= false
    ...
    override fun quickReject(left: Float, top: Float, right: Float, bottom: Float)= false
    ...
    override fun drawBitmap(bitmap: Bitmap, left: Float, top: Float, paint: Paint?)  = Unit
    ...
    override fun drawLines(pts: FloatArray, offset: Int, count: Int, paint: Paint)  = Unit
    ...
    override fun drawPoints(pts: FloatArray?, offset: Int, count: Int, paint: Paint) = Unit
    ...
    override fun drawText(text: String, start: Int, end: Int, x: Float, y: Float, paint: Paint)  = Unit
